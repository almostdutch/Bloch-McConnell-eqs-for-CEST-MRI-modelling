function [freq_offsets, spectrum] = cest_slave_sim_ps_fast(~, pools, freq_offsets, sampling_strategy, main_field_strength, ...
...
pulse_cest_int(1,2)=mt_pulse_int;
%% final spoiler
pulse_cest_sp(1,1)=0;
pulse_cest_sp(1,2)=final_spoiler;
%% readout
...
pulse_exc_dead_time(1,1)=0;
pulse_exc_dead_time(1,2)=seq_dead_time;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
for offset_No=1:length(ww)
    ...
    expmA_deadtime = zeros(length(pulse_exc_dead_time),length(pools)*3,length(pools)*3);
    Ainvbm_deadtime = zeros(length(pulse_exc_dead_time), length(pools)*3);
    
    for m=1:size(pulse_exc_dead_time,1)
        w1=2.0*pi*pulse_exc_dead_time(m,1);
        add_lineshape=zeros(1,length(pools));
        ...
        expmA_deadtime(m,:,:)= expm(A*pulse_exc_dead_time(m,2));
