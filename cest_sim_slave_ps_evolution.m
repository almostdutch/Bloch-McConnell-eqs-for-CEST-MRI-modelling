function varargout=cest_sim_slave_ps_evolution(spacing, pools, offset_w, pulse_method, cwpe_approximation, sampling, fs, B1amp, pulse_dur, pulse_int, spoiler,pulse_repeat, dummy, recovery, pulse_sampling_factor,readout_par,other_par)
...
%saturation_RF=ones(size(am_sg_100_100_0)); %% equivalent of a perfect rectangular pulse
...
elseif strcmp(cwpe_approximation,'true')==1
    pulse_cest_dur(1:Nsat,1)=1*sat_amp*42.576;
    pulse_cest_dur(1:Nsat,2)=sampling_rate_sat;
end
%% interval
pulse_cest_int(1:Nint,1)=0;
pulse_cest_int(1:Nint,2)=sampling_rate_sat;

% pulse_cest_int(1,1)=0;
% pulse_cest_int(1,2)=pulse_int;
%% final spoiler
pulse_sp(1:Nsp,1)=0;
pulse_sp(1:Nsp,2)=sampling_rate_sat;
...
% pulse_postexcitation_dead_time(1:Nrd,1)=0;
% pulse_postexcitation_dead_time(1:Nrd,2)=sampling_rate_ex;
...
Za_all=zeros(length(ww),((size(pulse_cest_dur,1)+size(pulse_cest_int,1))*pulse_repeat-size(pulse_cest_int,1)+size(pulse_sp,1)+(size(pulse_readout,1)+size(pulse_postexcitation_dead_time,1))*NofEx+1)*NofShots); % monitoring all points of H2O Zcomp
time_all=zeros(length(ww),((size(pulse_cest_dur,1)+size(pulse_cest_int,1))*pulse_repeat-size(pulse_cest_int,1)+size(pulse_sp,1)+(size(pulse_readout,1)+size(pulse_postexcitation_dead_time,1))*NofEx+1)*NofShots); % monitoring all points of H2O Zcomp
    ...
    for N_shots=1:NofShots
        ...
        if any(pulse_sp(:))>0
            for m=1:size(pulse_sp,1)
                ...
                Za_progress=[Za_progress abs(y(length(pools)*2+1))*100];
                time_progress=[time_progress pulse_sp(m,2)];
            end
        end
        ...
        for N_ex=1:NofEx
            ...
            for m=1:size(pulse_postexcitation_dead_time,1)
                ...
                Za_progress=[Za_progress abs(y(length(pools)*2+1))*100];
                time_progress=[time_progress pulse_postexcitation_dead_time(m,2)];
            end
        end
